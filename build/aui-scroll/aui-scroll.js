...
var Scroll = A.Component.create(
	{
		...
		ATTRS: {
			...
			/**
			 * Distance between the edge and the beginning of an area whose
		...
		prototype: {
			...
			_onScroll: function(event) {
				...
				if (scrolledUp) {
					...
					else if (hasTolerance && ((scrollTop - tolerance) < 0)) {
						instance.fire(DOWN + EDGE + APPROACHING, state);
					}
				...
				if (scrolledDown) {
					...
					else if (hasTolerance && ((scrollSnapY + tolerance) > 0)) {
						instance.fire(DOWN + EDGE + APPROACHING, state);
					}
				...
				if (scrolledLeft) {
					...
					else if (hasTolerance && ((scrollLeft - tolerance) < 0)) {
						instance.fire(DOWN + EDGE + APPROACHING, state);
					}
				...
				if (scrolledRight) {
					...
					else if (hasTolerance && ((scrollSnapX + tolerance) > 0)) {
						instance.fire(DOWN + EDGE + APPROACHING, state);
					}
